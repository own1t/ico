...
contract("ICO", (accounts) => {
  let ico;
  let token;
  const name = "Ryan Kim Token";
  const symbol = "RKT";
  const decimals = 18;
  const initialBalance = web3.utils.toBN(web3.utils.toWei("5000000"));
  ...
  it("should start the ICO", async () => {
    const duration = 100;
    const price = 1;
    const availableTokens = web3.utils.toWei("5000000");
    const minPurchase = web3.utils.toWei("100");
    const maxPurchase = web3.utils.toWei("10000");
  ...
  it("should NOT start the ICO", async () => {
    const duration = 100;
    const price = 1;
    let availableTokens = web3.utils.toWei("5000001");
    const minPurchase = web3.utils.toWei("100");
    let maxPurchase = web3.utils.toWei("10000");
    await expectRevert(
      ico.start(duration, price, availableTokens, minPurchase, maxPurchase),
      "totalSupply should be > 0 and <= totalSupply"
    );
    availableTokens = web3.utils.toWei("5000000");
    maxPurchase = web3.utils.toWei("5000001");
    await expectRevert(
      ico.start(duration, price, availableTokens, minPurchase, maxPurchase),
  ...
  context("Sale started", () => {
    ...
    it("should NOT let non-investors buy", async () => {
      await expectRevert(
        ico.buy({ from: accounts[3], value: web3.utils.toWei("1") }),
        "only investors"
      );
    });

    it("should NOT buy non-multiple of price", async () => {
      await ico.whitelist(accounts[3]);
